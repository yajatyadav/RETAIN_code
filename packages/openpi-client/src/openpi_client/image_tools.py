...
def save_debug_image(image: np.ndarray, idx: int, dir_name: str = "pre_resize_images"):
...
def resize_with_pad(images: np.ndarray, height: int, width: int, method=Image.BILINEAR) -> np.ndarray:
    ...
    #     save_debug_image(im, i, dir_name="pre_resize_images")
    resized = np.stack([_resize_with_pad_pil(Image.fromarray(im), height, width, method=method) for im in images])
    ret =  resized.reshape(*original_shape[:-3], *resized.shape[-3:])
    # for i, im in enumerate(resized):
    #     save_debug_image(im, i, dir_name="post_resize_images")
    return ret
