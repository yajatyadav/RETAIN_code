...
TRAINING_GPU_ALLOCATOR = "cuda_malloc_async"
...
def run_stage(gpu: int, config: str, exp_name: str, final_step: int) -> None:
    ...
    env = configure_jax_runtime(os.environ.copy())
    # Attempt 4 still fragmented the 95% BFC pool before its first optimizer
    # step.  The async allocator replaces the fixed BFC arena with a growing
    # CUDA pool, so stale preallocation controls must not leak into this run.
    for variable in (
    ...
    env.update(
        {
            "CUDA_DEVICE_ORDER": "PCI_BUS_ID",
            "CUDA_VISIBLE_DEVICES": str(gpu),
            "TF_GPU_ALLOCATOR": TRAINING_GPU_ALLOCATOR,
    ...
    status = {
        ...
        "jax_runtime": {
            ...
            "memory_policy": {
                "allocator": TRAINING_GPU_ALLOCATOR,
                "fixed_bfc_preallocation": False,
                "tf_force_gpu_allow_growth": True,
                "reason": (
                    "batch-64 在 95% BFC pool 下仍因 27.79 GiB 连续分配失败；"
                    "改用 JAX 官方文档建议的 growing cudaMallocAsync pool"
                ),
            },
        },
        "status": "running",
