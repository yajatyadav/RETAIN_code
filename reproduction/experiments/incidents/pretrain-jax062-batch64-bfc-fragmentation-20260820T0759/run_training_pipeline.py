    ...
    env.update(
        {
            "CUDA_DEVICE_ORDER": "PCI_BUS_ID",
            "CUDA_VISIBLE_DEVICES": str(gpu),
            # RTX 6000D has 83.64 GiB.  The first full batch-64 step compiled to
            # a 75.44 GiB peak, just above the 75.28 GiB pool created at 90%.
            # 95% leaves roughly 4 GiB above the compiled peak while retaining
            # headroom for the CUDA context and the input pipeline.
    ...
    status = {
        ...
        "jax_runtime": {
            ...
            "memory_policy": {
                ...
                "xla_python_client_preallocate": True,
                "tf_force_gpu_allow_growth": True,
                "reason": "batch-64 首步 XLA 峰值 75.44 GiB 略高于 90% 显存池 75.28 GiB",
            },
        },
        "status": "running",
    }
