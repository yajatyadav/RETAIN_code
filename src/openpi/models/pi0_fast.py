...
class Pi0FASTConfig(_model.BaseModelConfig):
    ...
    # TODO(YY): ALL VISION BACKBONE FREEZE CHANGES HELD IN THIS FUNCTION!!
    # TODO(YY): Ensure All Regexing correctly. The logic was done here so other models like pi0 can define their freeze filters differently and independently!!     
    def get_freeze_filter(self) -> nnx.filterlib.Filter:
        """ 
         Returns the freeze filter based on the model config.
         This method was modified to now freeze: 
         full-FT: 1) all img params if freeze_vision=True; else Nothing
         LoRA: 1) all LLM + img params, except those with "lora" if freeze_vision=True; else, all LLM params except those with "lora"
         """
        if "lora" in self.paligemma_variant:
            if self.freeze_vision:
                return nnx.All(nnx.Any(nnx_utils.PathRegex(".*llm.*"), nnx_utils.PathRegex(".*img.*")), nnx.Not(nnx_utils.PathRegex(".*lora.*")))
            return nnx.All(nnx_utils.PathRegex(".*llm.*"), nnx.Not(nnx_utils.PathRegex(".*lora.*")))
        if self.freeze_vision:
            return nnx.All(nnx_utils.PathRegex(".*/img/.*"))
        return nnx.Nothing
...
class Pi0FAST(_model.BaseModel):
    ...
    def sample_actions(
        ...
        *,
        ...
        # print("SAMPLE_ACTIONS CALLED with Observation: ", observation)
        # print("SAMPLE_ACTIONs Obs has image shape: ", observation.images["base_0_rgb"].shape, observation.images["wrist_0_rgb"].shape)
        observation = _model.preprocess_observation(
